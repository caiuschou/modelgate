...
└── src/
    ...
    │       └── copy-button.tsx        # 复制按钮（API 密钥复制等）
    │
    ├── features/                      # 功能模块（按业务划分）
    │   ├── dashboard/                 # 仪表盘
    │   │   ├── pages/
    ...
    │   │   └── types.ts
    │   │
    │   ├── api-keys/                  # API 密钥管理（与路由 /api-keys 对齐）
    │   │   ├── pages/
    │   │   │   └── token-list-page.tsx
    │   │   ├── components/
    │   │   │   ├── token-table.tsx
...
| `/api-keys` | ApiKeyListPage | 登录 | API 密钥列表 |
...
| 状态类型 | 管理方式 | 示例 |
|---------|---------|------|
| **服务端状态** | TanStack Query | 渠道列表、API 密钥列表、日志数据、统计数据 |
...
| POST | `/users/{username}/keys` | 创建 API Key | API 密钥管理 |
...
| GET | `/api/v1/tokens` | API 密钥列表（规划路径，以实际后端为准） | P0 |
| PUT | `/api/v1/tokens/{id}` | 更新 API 密钥 | P1 |
| DELETE | `/api/v1/tokens/{id}` | 删除/吊销 API 密钥 | P0 |
...
| E2E 测试 | Playwright | 关键业务流程（登录、创建 API 密钥、查看日志） | 10% |
