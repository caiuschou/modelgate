...
| 令牌管理 | 创建令牌、复制/吊销（页面落地后接入；当前部分路由为占位页可暂缓） |
...
### 6.4 令牌（`tokens.spec.ts`）

- 待 `/tokens` 非占位、接口联调完成后：创建 → 列表中出现 → 吊销确认流。
...
6. 接入 `register`（依赖邀请码）。  
7. 扩展 `logs`；若用例经令牌调用代理，确认审计/日志与 Mock 响应一致。令牌与 Admin 随功能交付增量补用例。  
